module FollowedResourceHelper
  ...
  def follow_button(resource)
    link_to(follow_path(:resource_id => resource.id, :resource_type => resource.class), :id => "follow_button", :class => " btn btn-mini" , "data-remote" => true, :method => :post) do
      content_tag(:i, "", :class => "icon-ok") + itext("follow")
    end
  end

  def unfollow_button(resource)
    link_to(unfollow_path(:resource_id => resource.id, :resource_type => resource.class), :id => "unfollow_button", :class => " btn btn-mini" , "data-remote" => true, :method => :delete) do
